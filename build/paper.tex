...
entangled nature of the browser platform.
...
failure. By design, every pageload results in a \textbf{global DOM tree}
managed by the browser engine. Regardless of rendering just a bunch of
static text nodes or an ever changing webapp the underlying global model
remains the same. \textbf{Encapsulation and modularization} is a typical
pattern in the computer industry to make complexity manageable and to
...
\textbf{complexe UI machine}. Without additional layers of abstraction
modularization would be impossible to archive.
...
separated HTML, CSS and JS in favor of an additional layer of
\textbf{abstracted JS components} (containing content, markup and
styling). Quiet often those frameworks mimic a MVC pattern on top of the
browser engine which is a reasonable simple design pattern to build
graphical user interfaces. This paper will apply the same pattern for
building a browsernative microservice.While frameworks are a valid
approach to build scalable web applications they remain highly
opinionated and can change and break over time. Furthermore, they
inflate the code with libraries as well. A standardized way for creating
complex UI interfaces painlessly requires new build-in browser
capabilities.
...
Calling the need for \emph{Browsernative Microservices} brings up the
...
comprehensive guide in \emph{Building Microservices} and the work of
Fowler and Lewis. The purpose of this section is to match those ideas
against the browser platform where different rules of development
applies.
...
of Fowler and Lewis: ``In short, the microservice architectural style is
...
signature and therefore highly accessible. HTML elements are usually
controlled via their attributes. A generic HTML service therefore might
look like this:
\texttt{\textless{}my-button\ isPrimary\textgreater{}\textless{}/button\textgreater{}}.

\subsection{Organized around Business
Capabilities}\label{organized-around-business-capabilities}

\subsection{Products not Projects}\label{products-not-projects}
