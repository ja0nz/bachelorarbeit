...
capacities. This paper should be read under the impression of
simplicity.
...
out as a objective manner - a goal software development should pursuit
and praise.
...
additional layers of abstraction can handle those complexity.

In the recent years many \textbf{frameworks}, libraries and
methodologies tried to tackle the global nature of the DOM by scoping
assets and design rules into maintainable components. While the DOM
can't be scoped, JS can. Therefore many of those frameworks, like
ReactJS, AngularJS or VueJS just to name a few, ditched the old rule of
separated HTML, CSS and JS in favor of an additional layer of abstracted
JS components (containing content, markup and styling). Quiet often
those frameworks mimic a MVC pattern on top of the browser engine which
is a reasonable simple design pattern to build graphical user
interfaces. This paper will apply the same pattern for building a
browsernative microservice.While frameworks are a valid approach to
build scalable web applications they remain highly opinionated, embody
inherent complexity themselves and can change and break over time.
Furthermore, they inflate the code with libraries. A standardized way
for creating complex UI interfaces painlessly requires new build-in
...
leapfrogged and many new low-level APIs brought to life. With the new
APIs at hand a webdeveloper can create scoped and highly reusable
microservices without additional libraries even directly in the browser
console.

\emph{Disclaimer:} Currently, all new technologies proposed throughout
this paper can be tried and tested in Google Chrome, Opera and the
Safari Technology Preview. Usually the browser adoption time is short in
time an therefore full support of all standards is expected soon.
Meanwhile this standards can be used through \textbf{polyfills} on every
important legacy browser.
...
Fowler and Lewis. The purpose of this section is to match those ideas
...
design} where we try to represent the real world in our code.{[}3, p.
2{]} Or making use of \emph{continuous delivery} for pushing software
rapidly through \emph{automated deployment} mechanisms in
production.{[}4{]} And, last but not least, microservices utilizes the
idea of small teams with a lot of product knowledge working mostly
...
replaceable and upgradeable.''{[}4{]} Compared to libraries, a component
service offers multiple advantages. A library is only loosely coupled to
the implementation and therefore hard to track in functionality.
Changing a library often requires an unforeseen amount of time fixing
implementations. Therefore in the field we often see multiple versions
of libraries deployed simultaneous (like with JQuery) which adds a huge
amount of overhead on the page load. Eventually, dead code elimination
won't work in highly coupled libraries. Components instead incorporate a
much better evolution mechanism in the service contracts. Changing
functionality won't break other components. A component can
progressively enhanced which guarantees functionality throughout
different versions.
...
signature and therefore highly accessible. HTML elements are explicitly
controlled via their attributes and the new HTML service component are
...
Microservice patterns are a product of real-world usage.{[}3, p. 1{]}
Instead of splitting teams along the technology stack (UI Experts
...
testing and maintaining their very own microservice. Across the stack
every member gains high competence about the functionality of the
service.

By assigning service responsibility a team, the so called
...
client-side is the most crucial area of business success. Todays
client-side includes many devices of differing sizes and
functionalities. As browser technologies, design guidelines and devices
change in a fast paced manner it makes absolutely sense to shift
responsibility towards the teams altogether. A \emph{browsernative
microservice} needs commitment over its whole lifecycle.

\subsection{Smart endpoints and dumb
pipes}\label{smart-endpoints-and-dumb-pipes}

Probably most microservices build for the browser are impure in
functionality. They aim for side-effects like changing the UI given to
input parameters or emitting browser events. Therefore a web developer
must design communication channels for those microservices keeping in
mind the nested nature of the DOM.

\emph{Smart endpoints and dumb pipes} is coined to the approach of
designing communication mostly decoupled and as cohesive as possible.
This essentially removes most of the error-prone middleware altogether.
In the analogy of the real world a message channel should look like
sending a letter: Only two endpoints (sender and receiver) connoted as
``smart'' and the letter itself looks mostly dumb (but unified) from the
outside. Only the receiver is able to process the content message.
...
communication protocols.{[}4{]} In the later parts of this paper the
reader will see a generic \emph{browsernative microservice} split in the
fashion of \textbf{MVC}. In the \textbf{VIEW} the microservice will rely
on browser events and in-memory calls while the \textbf{CONTROLLER} have
to deal with other resources over the fetch API.
